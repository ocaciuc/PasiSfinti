...
const CommentItem = memo(({ comment, userId, userBadges, onReplyAdded }: CommentItemProps) => {
  ...
  }, [replyText, userId, comment.id, comment.post_id, onReplyAdded]);

  const handleReplyEdited = useCallback((replyId: string, newContent: string) => {
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1">
          ...
          {editing ? (
            <div className="mt-1 space-y-1">
              ...
              <div className="flex gap-1">
                <Button onClick={handleSaveEdit} disabled={!editText.trim() || savingEdit} size="sm">
                  {savingEdit ? <Loader2 className="h-3 w-3 animate-spin" /> : "Salvează"}
                </Button>
                <Button variant="ghost" size="sm" onClick={() => { setEditing(false); setEditText(displayContent); }}>
                  Anulează
                </Button>
              </div>
            </div>
          ...
          <div className="flex items-center gap-3 mt-1">
            ...
            {isOwner && !editing && (
              <button
                onClick={() => { setEditing(true); setEditText(displayContent); }}
...
// Separate component for replies with edit support
      ...
      <div className="flex-1">
        ...
        {editing ? (
          <div className="mt-1 space-y-1">
            ...
            />
            <div className="flex gap-1">
              <Button onClick={handleSave} disabled={!editText.trim() || saving} size="sm">
