...
-- Update the notify_on_comment_reply trigger to also send a push notification
-- via the send-push edge function using pg_net
...
BEGIN
  ...
  -- Fallback: use hardcoded values if settings not available
  ...
  PERFORM net.http_post(
    ...
    body := jsonb_build_object(
      ...
      'body', reply_preview,
      'data', jsonb_build_object('type', 'comment_reply', 'post_id', NEW.post_id)
    )
  );
